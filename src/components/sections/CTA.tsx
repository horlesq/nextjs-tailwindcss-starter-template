...
interface CTAProps {
  variant?: "primary" | "consultation" | "portfolio" | "contact" | string;
}
...
export default function CTA({ variant = "primary" }: CTAProps) {
  ...
  // Cast cta to any to allow dynamic access since we don't have the explicit content type
  const ctaData = cta as any;
  
  // Pick the configured CTA or fall back to the first available entry
  const ctaContent =
    ctaData[key] || ctaData.primary || Object.values(ctaData)[0] || {};
